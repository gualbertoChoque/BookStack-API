...
@pytest.mark.e2e_Shelves
def test_TC001_shelves(get_url):
        """
        Descripción: Verificar que el todo el flujp de crear, obtener y eliminar un shelves se lleve de manera correcta y secuencial
        """
        payload = create_request_shelves_payload_super_modified()
        response_post = request_function(StaticDataVerbs.post.value, get_url, StaticDataModules.shelves.value,None,
        ...
        updated_payload = create_request_shelves_payload_super_modified(
            name="Shelf_Actualizado",
            description_html="Descripción modificada desde test automatizado"
        )
        response_put = request_function(StaticDataVerbs.put.value, get_url, StaticDataModules.shelves.value,
        ...
        assert response_put.json()["name"] == "Shelf_Actualizado"

        response_get = request_function(StaticDataVerbs.get.value, get_url, StaticDataModules.shelves.value,
        ...
        response_delete = request_function(StaticDataVerbs.delete.value, get_url, StaticDataModules.shelves.value,
                                     f"{shelf_id}", StaticDataHeaders.default_header.value
        )
